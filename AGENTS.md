...
- Prefer breaking dependency cycles and centralizing shared wiring in `src/dagsynth/core`; avoid long-lived shims.
...
- For behavior/schema changes under `src/dagsynth`, bump version in `pyproject.toml` (patch by default; minor for intentionally broad user-facing breaks). Docs/tests-only changes do not require a bump.
